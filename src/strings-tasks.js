...
function countVowels(str) {
  ...
  return str.split('').filter((ch) => 'aeiouyAEIOUY'.includes(ch)).length;
}
...
function extractNameFromTemplate(value) {
  ...
  const lastWord = splitedArr[splitedArr.length - 1];
  return lastWord.slice(0, lastWord.length - 1);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (upperCase.includes(char)) {
      const idx = upperCase.indexOf(char);
      const rotatedIdx = idx < 13 ? idx + 13 : idx - 13;
      result += upperCase[rotatedIdx];
    } else if (lowerCase.includes(char)) {
      const idx = lowerCase.indexOf(char);
      const rotatedIdx = idx < 13 ? idx + 13 : idx - 13;
      result += lowerCase[rotatedIdx];
    } else {
      result += char;
    }
  }
